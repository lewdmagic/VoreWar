...
class FogSystem
{
    ...
    internal void UpdateFog(Empire playerEmpire, Village[] villages, Army[] armies, List<GameObject> currentVillageTiles, List<GameObject> currentClaimableTiles)
    {
        ...
        foreach (Army army in StrategicUtilities.GetAllHostileArmies(playerEmpire))
        {
            ...
            if (spr != null)
                spr.gameObject.SetActive(!FoggedTile[army.Position.x, army.Position.y] && (!army.Units.All(u => u.HasTrait(Traits.Infiltrator)) || army.Units.Any(u => Equals(u.FixedSide, playerEmpire.Side))));
            var spr2 = army.Sprite;
            if (spr2 != null) spr2.enabled = !FoggedTile[army.Position.x, army.Position.y] && (!army.Units.All(u => u.HasTrait(Traits.Infiltrator)) || army.Units.Any(u => Equals(u.FixedSide, playerEmpire.Side)));
        }
