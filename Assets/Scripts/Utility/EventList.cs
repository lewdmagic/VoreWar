...
internal class EventList
{
    ...
    internal void CheckStartEvent(Empire empire)
    {
        ...
        if (empire.RecentEvents == null) empire.RecentEvents = new List<int>();
        for (int i = 0; i < 20; i++)
        {
            int val = State.Rand.Next(30);
            if (i < 17 && empire.RecentEvents.Contains(val)) continue;
            if (Config.EventsRepeat == false && empire.EventHappened.ContainsKey(val)) continue;
            try
            {
                if (State.EventList.StartEvent(val, empire))
                {
                    ...
                    if (Config.EventsRepeat && empire.RecentEvents.Count() > 8) empire.RecentEvents.RemoveAt(0);
                    triggered = true;
                    break;
                }
            }
    ...
    {
        if (CustomEvents.AnyEvents == false) return;
        ...
        _ui.FirstChoice.onClick.AddListener(() =>
        {
            ...
            if (result.EndsWith(", ")) result = result.TrimEnd(new char[] { ',', ' ' });
            result += ")";
            if (result == "()") result = "";
    ...
    private bool StartEvent(int num, Empire empire)
    {
        switch (num)
        {
            ...
            {
                var hostileEmpire = GetRandomHostileEmpire(empire);
                if (hostileEmpire == null) hostileEmpire = GetRandomEmpire(empire);
                Village village;
    ...
    internal void CheckStartAIEvent()
    {
        ...
        if (empires == null) return;
        Empire first = empires[0];
        Empire second = empires[1];
        int num = State.Rand.Next(30);
        int time = 90;
        if (State.GameManager.StrategyMode.OnlyAIPlayers) time = 12;
        if (Config.AIToAIMessagesForever) time = 0;
        switch (num)
        {
            case 0:
    ...
    private void CreateBandits(RebelDifficulty diff, Village village, Race race)
    {
        ...
        if (count == 0) return;
        if (count >= village.Empire.MaxArmySize) count = village.Empire.MaxArmySize;
    ...
    private void CreateRebels(RebelDifficulty diff, Village village)
    {
        ...
        if (count >= village.Population) count = village.Population;
        if (count >= village.Empire.MaxArmySize) count = village.Empire.MaxArmySize;
        if (count == 0) return;
    ...
    private Empire GetRandomHostileEmpire(Empire empire)
    {
        ...
        if (hostileEmpires.Count() == 0) return null;
    ...
    private Empire GetRandomAlliedEmpire(Empire empire)
    {
        ...
        if (alliedEmpires.Count() == 0) return null;
    ...
    private Empire[] GetTwoRandomEmpires(Empire empire)
    {
        ...
        if (hostileEmpires.Length <= 1) return null;
